...
for line in f:
	...
	myValue += gyro_z*(ts_now-ts_prev) #-0.000100176113876
	myValue2 += gyro_z*(0.01) #-0.000101684399899 #Not using the timestamps! not good! the datarate is 100Hz, therefore 0.01s

	# in order to show a plot use this function to append your value to a list:
	plotData.append (myValue*180.0/pi)
	plotData_2.append (myValue2*180.0/pi)
...
print "Bias for each sample is approx (ind timesteps): ",((plotData[len(plotData)-1]/(180.0/pi))-(plotData[0]/(180.0/pi)))/len(plotData)
print "Bias for each sample is approx (constant time): ",((plotData_2[len(plotData)-1]/(180.0/pi))-(plotData_2[0]/(180.0/pi)))/len(plotData_2)

# show the plot
if showPlot == True:
	plt.plot((0, len(plotData)), (plotData[0], plotData[0]), 'r-')
	plt.plot(plotData, 'b-', label="Relative angle Z-axis; individual timestamps")
	plt.plot(plotData_2, 'g-', label="Relative angle Z-axis; constant sample time")
	plt.legend(loc=2, borderaxespad=0.) #bbox_to_anchor=(0.95, 1)
